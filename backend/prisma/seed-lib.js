...
exports.CATALOGO_MODULOS = exports.CATALOGO_PERMISOS = void 0;
exports.seedEmpresaBase = seedEmpresaBase;
const bcrypt_1 = __importDefault(require("bcrypt"));
exports.CATALOGO_PERMISOS = [
...
exports.CATALOGO_MODULOS = [
    ...
    },
    {
        clave: 'nomina',
        nombre: 'Nómina',
        descripcion: 'Registro simple de pagos a empleados por periodo (sin cálculo automático de IESS ni décimos)',
        precioMensual: 7,
    },
];
async function seedEmpresaBase(opts) {
    const { prisma, empresaNombre, dominio, adminNombre, adminEmail, adminPassword, activarModuloClientes = true, modulosActivos = [], } = opts;
    const permisos = await Promise.all(exports.CATALOGO_PERMISOS.map((permiso) => prisma.permiso.upsert({
        where: { clave: permiso.clave },
        update: {},
        create: permiso,
    })));
    const modulos = await Promise.all(exports.CATALOGO_MODULOS.map((modulo) => prisma.modulo.upsert({
    ...
    if (activarModuloClientes)
        clavesAActivar.add('clientes');
